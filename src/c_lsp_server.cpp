// `mep-c-lsp`: the wire half of mep's own C language server. Speaks LSP
// over stdio; every answer comes from c_lsp.h's pure analysis functions,
// which is where the actual C knowledge lives (c_ast.h is the lexer,
// preprocessor model and parser under those).
//
...
//
// The thing this server does *not* do is the thing every other C tool
// does first: it never runs the preprocessor and never asks for a
// compilation database. An editor is looking at unpreprocessed source,
// and a server that needs `-I` flags to say anything says nothing in
// most buffers. c_ast.h documents what is done instead, and c_lsp.h
// documents what that costs.
//
// Deliberately single-threaded and synchronous. Every request is a parse
// plus a walk of one already-in-memory document -- a few milliseconds
// for files far larger than anyone edits by hand -- so there is no work
// queue, no cancellation handling and no background indexing to get
// wrong.
//
// Transport notes:
...
//     match what mep's client reads `character` as. C source is full of
//     non-ASCII string literals and comments, so declaring UTF-16 and
...
public:
    ...
    void HandleMessage(const std::string &body) {
        ...
        if (method == "workspace/didChangeConfiguration") {
            const Json &settings = params.get("settings").get("c");
...
private:
    ...
    static Json Capabilities() {
        ...
        // the two member operators, and the `<`, `"` and `/` of an
        // `#include` line, whose candidates are file names rather than
        // identifiers.
    ...
    /** @brief Builds the analysis options for a document URI (its directory anchors the `#include "..."` lookup). */
    CLspOptions OptionsFor(const std::string &uri) const {
        CLspOptions opts;
        ...
        // `#include "config.h"` to a file that has nothing to do with
        // it.
    ...
    void Publish(const std::string &uri) {
        const std::vector<CLspDiagnostic> diags = CLspDiagnostics(Doc(uri), OptionsFor(uri));
        Json arr = Json::Array();
    ...
    Json Completion(const Json &params) const {
        ...
        const std::vector<CLspCompletionItem> items =
            CLspCompletions(Doc(req.uri), req.line, req.col, OptionsFor(req.uri));
        Json arr = Json::Array();
        for (size_t i = 0; i < items.size(); i++) {
            const CLspCompletionItem &it = items[i];
            Json j = Json::Object();
            j["label"] = it.label;
            j["kind"] = static_cast<int>(it.kind);
            j["insertText"] = it.insert_text;
            j["insertTextFormat"] = 1;  // PlainText: never a snippet, see c_lsp.h
    ...
    Json HoverAt(const Json &params) const {
        const Request req = ReadRequest(params);
        const CLspHoverInfo info = CLspHover(Doc(req.uri), req.line, req.col);
    ...
    static Json SymbolNode(const std::vector<CLspSymbol> &syms, const std::vector<std::vector<int>> &children,
                           const std::vector<std::string> &lines, size_t index) {
        const CLspSymbol &s = syms[index];
        Json j = Json::Object();
        j["name"] = s.name;
    ...
    Json Symbols(const Json &params) const {
        ...
        const std::vector<CLspSymbol> syms = CLspSymbols(lines);
    ...
    Json Folds(const Json &params) const {
        const std::string uri = params.get("textDocument").get("uri").as_string();
        Json arr = Json::Array();
        for (const CLspFold &f : CLspFolds(Doc(uri))) {
            Json j = Json::Object();
    ...
    Json Definition(const Json &params) const {
        const Request req = ReadRequest(params);
        const CLspLocation loc = CLspDefinition(Doc(req.uri), req.line, req.col, OptionsFor(req.uri));
    ...
    Json References(const Json &params) const {
        const Request req = ReadRequest(params);
        const CLspReferenceSet refs = CLspReferences(Doc(req.uri), req.line, req.col);
        const bool include_decl = !params.get("context").is_object() ||
                                  params.get("context").get("includeDeclaration").as_bool();
        Json arr = Json::Array();
        for (const CLspReference &r : refs.refs) {
    ...
    Json SignatureHelp(const Json &params) const {
        const Request req = ReadRequest(params);
        const CLspSignature sig = CLspSignatureHelp(Doc(req.uri), req.line, req.col);
