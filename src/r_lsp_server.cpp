// `mep-r-lsp`: the wire half of mep's own R language server. Speaks LSP
// over stdio; every answer comes from r_lsp.h's pure analysis functions,
// which is where the actual R knowledge lives.
//
...
// subprocess rather than the editor, and an .R file edited in any other
// LSP client gets the same diagnostics.
//
// Two things make this server unusual among R tooling, and both are
// deliberate:
//   - It never starts R. No `languageserver`, no `lintr`, no session to
//     keep alive, nothing to install -- which means it answers on a
//     machine with no R at all, and answers immediately rather than
//     after a package load.
//   - It never evaluates the document. Every answer is a property of the
//     source text, so opening a script cannot run it. That rules out
//     knowing what a variable holds at run time, which is the honest
//     trade: see r_lsp.h's scope note.
//
...
/** @brief Builds the analysis options for a document URI (its directory anchors `source()` paths). */
RLspOptions OptionsFor(const std::string &uri) {
    RLspOptions opts;
...
Json RangeOf(const RRange &range) {
    Json r = Json::Object();
...
private:
    ...
    static Json Capabilities() {
        ...
        // The characters that open an R construct with nothing
    ...
    void Publish(const std::string &uri) {
        const std::vector<RLspDiagnostic> diags = RLspDiagnostics(Doc(uri), OptionsFor(uri));
        Json arr = Json::Array();
    ...
    Json Completion(const Json &params) const {
        ...
        const std::vector<RLspCompletionItem> items =
            RLspCompletions(Doc(uri), line, ColOf(params), OptionsFor(uri));
        Json arr = Json::Array();
        for (size_t i = 0; i < items.size(); i++) {
            const RLspCompletionItem &it = items[i];
            Json j = Json::Object();
            j["label"] = it.label;
            j["kind"] = static_cast<int>(it.kind);
            j["insertText"] = it.insert_text;
            j["insertTextFormat"] = 1;  // PlainText: never a snippet, see r_lsp.h
    ...
    Json HoverAt(const Json &params) const {
        const RLspHoverInfo info = RLspHover(Doc(UriOf(params)), LineOf(params), ColOf(params));
    ...
    Json Folds(const Json &params) const {
        Json arr = Json::Array();
        for (const RLspFold &f : RLspFolds(Doc(UriOf(params)))) {
            Json j = Json::Object();
    ...
    Json Definition(const Json &params) const {
        const std::string uri = UriOf(params);
        const RLspLocation loc = RLspDefinition(Doc(uri), LineOf(params), ColOf(params), OptionsFor(uri));
    ...
    Json References(const Json &params) const {
        ...
        Json arr = Json::Array();
        for (const RLspReference &ref : RLspReferences(Doc(uri), LineOf(params), ColOf(params))) {
    ...
    Json Highlights(const Json &params) const {
        Json arr = Json::Array();
        for (const RLspReference &ref : RLspReferences(Doc(UriOf(params)), LineOf(params), ColOf(params))) {
            Json j = Json::Object();
            j["range"] = RangeOf(ref.range);
    ...
    Json SignatureHelp(const Json &params) const {
        const RLspSignature sig = RLspSignatureHelp(Doc(UriOf(params)), LineOf(params), ColOf(params));
...
/**
 * @brief Runs the R language server's stdio read loop until the client sends `exit` (or closes the stream).
